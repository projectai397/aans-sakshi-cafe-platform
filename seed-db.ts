...
import { articles, subscribers, divisionContent } from "./drizzle/schema";
  ...
  try {
    ...
    for (const article of articles_data) {
      await db.insert(articles).values({
        ...
        division: article.division as any,
    ...
    for (const div of divisions_data) {
      await db.insert(divisionContent).values({
        division: div.division.toLowerCase() as any,
        title: div.name,
        description: div.description,
        features: div.features,
        metrics: `Revenue: ${div.revenue}, Active Users: ${div.activeUsers}`,
      });
    }
